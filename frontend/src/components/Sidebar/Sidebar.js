...
const Sidebar = ({ open, onClose, onToggle }) => {
  ...
  const isFinancial = userRole === 'FINANCIAL' || isAdmin;
  const isAuxAdmin = userRole === 'ADMINISTRATIVE_ASSISTANT' || isFinancial;
  const isSupportN2 = userRole === 'SUPPORT_N2' || isAdmin;
  const isSupportAnalyst = userRole === 'SUPPORT_ANALYST' || isSupportN2 || isAdmin;

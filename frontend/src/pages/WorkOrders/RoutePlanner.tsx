...
const RoutePlanner = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const scheduledOrders = ((woRes as any)?.data || woRes || []).filter(
        (w: any) => w.status === 'SCHEDULED' || w.status === 'PENDING_SCHEDULE'
      );
      setWorkOrders(scheduledOrders);
      setSelectedIds(scheduledOrders.map((w: any) => w.id));

      const routes = (routesRes as any)?.data || routesRes || [];
      if (routes && routes.length > 0) {
        const latestRoute = routes[0];
        setCurrentRoute(latestRoute);
        const stopsRes: any = await geoCepService.getStopsByRouteId(latestRoute.id);
        setRouteStops(stopsRes?.data || stopsRes || []);
      }
    } catch (err) {

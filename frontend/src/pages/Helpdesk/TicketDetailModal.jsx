...
const TicketDetailModal = ({ open, onClose, ticket, onUpdated, currentUserRole = 'ATTENDANT', currentUserId = null, currentUserName = 'Atendente' }) => {
  ...
  const loadInteractions = async () => {
    ...
    try {
      setLoading(true);
      const res = await helpdeskService.getInteractions(ticket.id, true);
  ...
  const handleEscalateToN2 = async () => {
    ...
    try {
      setSubmitting(true);
      await helpdeskService.escalateToN2(ticket.id, {
        attendantUserId: currentUserId,
        attendantName: currentUserName,
  ...
  const handleResolveByN2 = async () => {
    ...
    try {
      setSubmitting(true);
      await helpdeskService.resolveByN2(ticket.id, {
  ...
  const handleEscalateToWorkOrder = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await helpdeskService.escalateToWorkOrder(ticket.id, {
        n2UserId: currentUserId,
        n2Name: currentUserName,
        technicalReason: woReason,
      });
      toast.success(`Ordem de Serviço #${res.data.id?.substring(0, 8)} gerada para a equipe de campo!`);

    ...
    <Box sx={{ p: 3, maxWidth: 1400, margin: '0 auto' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Emissão de Notas Fiscais Eletrônicas de Telecomunicações (SCM), Certificados Digitais A1 e Convênio ICMS 115/03.
          </Typography>
        </Box>
      ...
      <Paper sx={{ mb: 3 }}>
        <Tabs value={activeTab} onChange={(e, val) => setActiveTab(val)} indicatorColor="primary" textColor="primary">
          ...
          <Tab icon={<ZipIcon />} iconPosition="start" label="Convênio ICMS 115/03" />
        </Tabs>
      </Paper>
      ...
      {/* TAB 2: CONVÊNIO ICMS 115/03 */}
      {activeTab === 2 && (
        <Card>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
              <ZipIcon color="primary" /> Exportação de Arquivos Magnéticos (Convênio ICMS 115/03)
            </Typography>
            ...
            <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 2, border: '1px solid', borderColor: 'grey.200' }}>
              <Typography variant="body2" sx={{ fontWeight: 'bold', mb: 1 }}>
                📋 Conteúdo do Pacote ZIP Gerado:
              </Typography>
              <Typography variant="caption" component="div" sx={{ fontFamily: 'monospace', color: 'text.secondary' }}>
                • PA{companyForm.cnpj.replace(/\D/g, '')}62001{String(convenioYear).slice(2)}{String(convenioMonth).padStart(2, '0')}N01.M (Arquivo Mestre de Faturas)
              </Typography>
              <Typography variant="caption" component="div" sx={{ fontFamily: 'monospace', color: 'text.secondary' }}>
                • PA{companyForm.cnpj.replace(/\D/g, '')}62001{String(convenioYear).slice(2)}{String(convenioMonth).padStart(2, '0')}N01.I (Arquivo de Itens Faturados)
              </Typography>
              <Typography variant="caption" component="div" sx={{ fontFamily: 'monospace', color: 'text.secondary' }}>
                • PA{companyForm.cnpj.replace(/\D/g, '')}62001{String(convenioYear).slice(2)}{String(convenioMonth).padStart(2, '0')}N01.D (Arquivo de Destinatários)
              </Typography>
              <Typography variant="caption" component="div" sx={{ fontFamily: 'monospace', color: 'text.secondary' }}>
                • PA{companyForm.cnpj.replace(/\D/g, '')}62001{String(convenioYear).slice(2)}{String(convenioMonth).padStart(2, '0')}N01.C (Arquivo de Controle com Hashes MD5)
              </Typography>
            </Box>
          </CardContent>
        </Card>

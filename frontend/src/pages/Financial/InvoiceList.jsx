...
const InvoiceList = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [invRes, conRes, custRes] = await Promise.all([
        invoiceService.getAllInvoices(),
        contractService.getAllContracts().catch(() => ({ data: [] })),
  ...
  const handleCreateManualInvoice = async (e) => {
    ...
    try {
      await invoiceService.generateInvoiceManually(newInvoiceForm.contractId, newInvoiceForm.dueDate);
  ...
  const handleProcessDailyDunning = async () => {
    try {
      ...
      const res = await billingDunningService.processDailyDunning();
      setDunningResult(res.data);
      toast.success(`Régua de cobrança executada! ${res.data.suspendedCount} contratos suspensos por inadimplência.`);
      loadData();
    } catch (err) {
  ...
  const handleExecuteAttendantUnblock = async (e) => {
    ...
    try {
      const res = await billingDunningService.requestAttendantUnblock(
